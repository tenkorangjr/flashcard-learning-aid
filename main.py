...
def reset_window():
    ...
    canvas.itemconfig(card_text, text=f"{rand_dict['French']}", fill="black")
    window.update()
...
# UI INTERFACE
window = Tk()
window.title("Michael's FlashCards")
...
lang_text = canvas.create_text(400, 170, text="French", font=("Arial", 30, "italic"))
card_text = canvas.create_text(400, 280, font=("Arial", 40, "bold"), text=f"{rand_dict['French']}")
...
word_to_learn_list.to_csv("words_to_learn.csv", columns=["French", "English"])
